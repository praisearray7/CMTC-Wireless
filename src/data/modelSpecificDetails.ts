...
CMTC Wireless is your most trusted mobile device repair shop in the Twin Cities.`,
  image: '/images/repair-placeholder.jpg', // Fallback
  keywords: ['repair', 'fix', 'service', 'replacement'],
  desc: 'Professional repair service for your device.' // Fallback short desc
};
...
/**
 * Get repair detail information for any repair type.
 * If not explicitly defined, generates a dynamic fallback.
 */
export function getRepairDetail(repairType: string, index?: number): RepairDetail {
  // Return explicit definition if it exists
  if (repairDetails[repairType]) {
    return repairDetails[repairType];
  }

  // Default images for dynamic repairs - cycle through them
  const defaultImages = [
  ...
  // Auto-generate for unknown repair types
  const title = repairType
    .split('-')
    .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
    .join(' ');

  // Simple cycling - use index if provided, otherwise 0
  const imageIndex = (index !== undefined ? index : 0) % defaultImages.length;

  return {
    title,
    desc: `Professional ${title.toLowerCase()} service with premium parts and warranty.`,
